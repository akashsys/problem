...
async def test_add(dut):
    ...
    assert int(dut.result.value) == 15, "ADD operation failed"
...
async def test_sub(dut):
    ...
    assert int(dut.result.value) == 13, "SUB operation failed"
...
async def test_and(dut):
    ...
    assert int(dut.result.value) == (0xF0F0 & 0x0FF0), "AND operation failed"
...
async def test_mul(dut):
    ...
    assert int(dut.result.value) == 120, "MUL operation failed"
...
async def test_div(dut):
    ...
    assert int(dut.result.value) == 25, "DIV operation failed"
...
async def test_sll_operation(dut):
    ...
    assert int(dut.result.value) == (1 << 4), \
        "SLL operation failed (functional shift-left bug)"
...
async def test_clamp_is_valid_value(dut):
    ...
    assert val.is_resolvable, "Clamp value is X or Z"
    assert int(val) in [0, 1], "Clamp must be 0 or 1"


@cocotb.test()
async def test_output_not_xz_and_not_alu_when_diss_clk(dut):
    await start_clock(dut)
    await reset_dut(dut)

    await run_op(dut, 9, 6, 0, 2)

    alu_val = int(dut.result.value)

    dut.diss_clk.value = 1
    await RisingEdge(dut.clk)

    val = dut.result.value
    assert val.is_resolvable, "Result is X or Z when diss_clk is applied"

    assert int(val) == int(dut.clamp_obs.value), \
        "Result is not clamped when diss_clk is applied (ALU output leaked)"



@cocotb.test()
async def test_diss_clk_clamps_output(dut):
    await start_clock(dut)
    await reset_dut(dut)
    ...
    assert int(dut.result.value) == int(dut.clamp_obs.value), \
        "Result does not match clamp value when diss_clk is applied"
